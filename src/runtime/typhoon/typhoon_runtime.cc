...
class TyphoonRuntimeState {
 public:
  ...
  int ReplayCapturedLayer(int32_t graph_id, int32_t layer_id, int32_t replay_kind) {
    return Call([&]() {
      ...
      pending.replay_kind = replay_kind;
    });
  }
 ...
 private:
  ...
  static void FlushPendingReplayLayer(GraphRuntimeState* state) {
    auto& pending = state->replay.pending_layer;
    if (!pending.present) {
      return;
    }
    if (state->replay.skipped_layer_ids.count(pending.layer_id) != 0) {
      pending = CanonicalReplayState::PendingLayer{};
      return;
    }
    state->graph.SetCurrentLayerId(pending.layer_id);
